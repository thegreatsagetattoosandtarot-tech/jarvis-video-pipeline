#!/usr/bin/env python3
"""
J.A.R.V.I.S. OS - Hostinger Backup Sync Service
Synchronizes J.A.R.V.I.S. OS system to Hostinger server via SCP over SSH.
"""
...
def load_config():
    ...
    if not CONFIG_FILE.exists():
        template = {
            ...
            "ssh_key": "/opt/data/JARVIS_OS/config/secrets/hostinger_key",
            "port": 65002,
            "retention_days": 30,
...
def create_backup_archive(config, incremental=False) -> Path:
    ...
    archive_name = f"jarvis_os_backup_{timestamp}.tar.gz"
...
def verify_backup(config) -> bool:
    ...
    ssh_cmd = [
        ...
        "-o", "UserKnownHostsFile=/dev/null",
        f"{user}@{host}",
        f"ls -lh {remote_path}/jarvis_os_backup_*.tar.gz 2>/dev/null | tail -5"
    ]
...
def cleanup_old_backups(config):
    ...
    ssh_cmd = [
        ...
        "-o", "UserKnownHostsFile=/dev/null",
        f"{user}@{host}",
        f"find {path} -name 'jarvis_os_backup_*.tar.gz' -mtime +{retention} -delete"
    ]
...
def main():
    parser = argparse.ArgumentParser(description="J.A.R.V.I.S. OS Hostinger Backup Sync (SCP)")
