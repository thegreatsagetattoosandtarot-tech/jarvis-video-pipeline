...
from agents.metatron import MetatronAgent

# Import all agent modules (they self-register)
try:
    from agents import gabriel, raphael, uriel, sandalphon, jophiel
    from agents import michael, raguel, remiel, sariel, azrael, chamuel
except ImportError:
...
async def spawn_agent(agent_id: str) -> bool:
    ...
    try:
        ...
        agent_dir = Path("/opt/data/jarvis/config/agents") / agent_id
...
async def spawn_all():
    ...
    for agent_id, name, archangel, role, domain, sintra_parallel, tools in agents_config:
        if agent_id not in AGENT_REGISTRY:
            ...
            from agents.metatron import MetatronAgent
            AGENT_REGISTRY[agent_id] = (MetatronAgent, config)
    ...
    # Save registry
    registry_file = Path("/opt/data/jarvis/config/agents/registry.json")
...
async def stop_all():
    ...
    agents_dir = Path("/opt/data/jarvis/config/agents")
